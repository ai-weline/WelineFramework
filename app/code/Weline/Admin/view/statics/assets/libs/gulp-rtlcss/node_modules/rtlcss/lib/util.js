...
module.exports = {
    ...
    swap: function (value, a, b, options) {
        ...
        return value.replace(new RegExp(expr, flags), function (m) {
            return compare(m, a, options.ignoreCase) ? b : a
        })
    },
    ...
    negate: function (value) {
        ...
        state.value = state.value.replace(REGEX_NEGATE_ONE, function (num) {
            return REGEX_TOKEN_REPLACEMENT.test(num) ? num.replace(REGEX_TOKEN_REPLACEMENT, function (m) {
                return '(-1*' + m + ')'
            }) : parseFloat(num, 10) * -1
        })
    ...
    guard: function (what, who, indexed) {
        ...
        if (state.indexed === true) {
            while (what.test(state.value)) {
                state.value = state.value.replace(what, function (m) {
                    state.store.push(m);
