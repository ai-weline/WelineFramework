...
if (!shouldBreak) {
    if (!output && input !== '-') {
        if (directory !== true) {
            output = path.extname(input) ? input.replace(/\.[^.]*$/, function (ext) {
                return '.rtl' + ext
            }) : input + '.rtl'
        } else {
            output = input
    ...
    var processCSSFile = function (e, data, outputName) {
        ...
        if (output) {
            ...
            fs.writeFile(savePath, result.css, 'utf8', function (err) {
                err && printError(err)
            })
            if (result.map) {
                fs.writeFile(savePath + '.map', result.map, 'utf8', function (err) {
                    err && printError(err)
                })
            }
        } else {
