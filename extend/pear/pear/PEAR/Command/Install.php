...
'],
     ...
     */
    public function &getDownloader(&$ui, $options, &$config)
    {
    ...
    public function enableExtension($binaries, $type)
    {
    ...
    public function disableExtension($binaries, $type)
    {
        if (!($phpini = $this->config->get('php_ini', null, 'pear.php.net'))) {
            return PEAR::raiseError('configuration option "php_ini" is not set to php.ini location');
        }
    ...
    public function _parseIni($filename)
    {
    ...
    public function doInstall($command, $options, $params)
    {
    ...
    */
    ...
    public function doBundle($command, $options, $params)
    {
        $opts = [
            'force'        => true,
            'nodeps'       => true,
    ...
    public function doRunScripts($command, $options, $params)
    {
        if (!isset($params[0])) {
     ...
     */
    public function _filterUptodatePackages($packages, $command)
    {
        $reg = &$this->config->getRegistry();
        $latestReleases = [];
