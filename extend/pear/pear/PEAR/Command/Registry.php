...
installed package.'
        ...
        if ($info->getPackagexmlVersion() == '1.0' || $installed) {
            foreach ($list as $file => $att) {
                if ($installed) {
                    ...
                    if (isset($att['baseinstalldir']) && !in_array(
                        $att['role'],
                        ['test', 'data', 'doc']
                    )) {
    ...
    public function doShellTest($command, $options, $params)
    {
        if (count($params) < 1) {
     ...
     * @access private
     */
    public function _doInfo2($command, $options, $params, &$obj, $installed)
    {
        $reg = &$this->config->getRegistry();
