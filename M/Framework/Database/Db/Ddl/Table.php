...
class Table
{
     ...
     * @return Table
     */
    function addConstraints(string $constraints = ''): Table
    {
    ...
    public function create()
    {
        ...
        foreach ($this->_fields as $field) {
            if (end($this->_fields) == $field) {
                ...
                if (empty($this->_indexes)) $field = trim($field, ',');// 如果没有设置索引
            };
            $fields_str .= $field;
        }
        // 索引
        ...
        foreach ($this->_indexes as $index) {
            if (end($this->_indexes) == $index) {
                if (empty($this->constraints)) $index = trim(trim($index, PHP_EOL), ',');
            };
            $indexes_str .= $index;
        }
        $sql = <<<createSQL
