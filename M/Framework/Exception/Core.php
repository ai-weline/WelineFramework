...
class Core extends \Exception
{
    ...
    function getErrorCode()
    {
        ...
        $returnTxt = $isCli ? $startColor : '<div style="padding:25px;color:#767678;background-color:#252729;margin: 15px 8px 8px 8px">'; // 初始化返回
        $i = 1; // 行数
        $start_line = $this->line - 5;
        $end_line = $this->line + 5;
        while (!feof($fileSource)) {
            $buffer = fgets($fileSource);
            $buffer = $isCli ? $buffer : str_replace(' ', '&nbsp;', $buffer);
            $line = $isCli ? $i . '# ' : '<b style="color: gray">' . $i . '</b>';
            // 指定行范围读取
            if ($i > $start_line && $i < $end_line) {
                if ($isCli) {
                    if ($this->line == $i) $buffer = $endColor . $heightColor . $buffer . $endColor . $startColor;
                    $returnTxt .= $line . $buffer . PHP_EOL;
                } else {
                    if ($this->line == $i) $buffer = '<b style="font-weight: bolder;color:#a01b00">' . $buffer . '</b>';
